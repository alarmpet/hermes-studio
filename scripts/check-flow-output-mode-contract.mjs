...
assert.match(outputModeHelper, /Imagen 4/, "image mode should still know fallback model labels for diagnostics");
assert.match(outputModeHelper, /IMAGE_MODEL_LABELS/, "image mode should choose models through an explicit allow-list");
assert.match(outputModeHelper, /disableAgentMode/, "Flow image mode should turn the bottom Agent selector off before selecting image settings");
assert.match(outputModeHelper, /agentModeOffRequired/, "Flow image mode should require Agent mode off");
assert.match(outputModeHelper, /\uc548\ud568|\uc0ac\uc6a9 \uc548\ud568|none|off/, "Flow agent selector should know off labels");
...
assert.match(outputModeHelper, /imageModel:\s*\[[^\]]*"Nano Banana Pro"/s, "image mode should default to Nano Banana Pro");
...
assert.match(flow, /FLOW_OUTPUT_SETTINGS_NOT_CONFIRMED/, "Flow automation should fail before submit when settings clicks are not fully confirmed");
assert.match(flow, /FLOW_IMAGE_SETTINGS_MISMATCH/, "Flow automation should hard-gate requested image model, aspect ratio, and count");

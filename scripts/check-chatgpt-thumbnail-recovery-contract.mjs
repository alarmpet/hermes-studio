...
assert.match(service, /primaryProviderFailure/, "thumbnail-only retry should preserve primary provider failure details");
assert.match(service, /actionRequired:\s*thumbnailActionRequired/, "thumbnail recovery should surface user-action guidance to the progress UI");
assert.match(service, /Open Authenticate Google Flow/, "thumbnail recovery should tell the user to complete Google Flow verification manually");
...
assert.match(renderer, /FLOW_THUMBNAIL_GENERATION_FAILED|CHATGPT_HUMAN_VERIFICATION_REQUIRED/, "renderer should keep explicit thumbnail recovery guidance");
